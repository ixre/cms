...
var RIBBON = {
    ...
    eachPage: function (call) {
        ...
        J.each(this.tabPages, function (i, e) {
    ...
    init: function (e) {
        ...
        J.each(this.tabPages, function (i, e) {
            e.onclick = (function (_i) {
                return function () {
                    J.each(R.tabPages, function (j, je) {
...
function menu_init() {
    ...
    J.each(J.getElementsByClassName(RIBBON.ele.source, 'fn'), function (i, e) {
        e.onclick = function () {
...
RIBBON.resize = function () {
    ...
    if (sib != null) {
        ...
        J.$('ribbon-main-right').style.width = (width - J.$('ribbon-main-left').offsetWidth) + 'px';
    }
};
...
var tab = {
    ...
    initialize: function () {
        ...
        this.frameHeight = framebox.offsetHeight;
    ...
    show: function (text, url) {
        ...
        if (!_exits) {
            ...
            J.event.add(frame, 'load', _loadCall);
    ...
    set: function (t) {
        ...
        var offsetHeight = this.frameHeight;
    ...
    close: function (t) {
        ...
        cms.each(J.getElementsByClassName(this.tabs, 'closebtn'), function (i, obj) {
            if (obj == t) {
                closeIndex = i + 1;
...
function initSites() {
    ...
    if (window.sites.length == 0) {
        ...
        J.each(siteEle.getElementsByTagName('DIV')[0].getElementsByTagName('LI'), function (i, e) {
...
//加载用户状态
J.$('custom_menu').innerHTML = J.$('headerOper').innerHTML;
J.hover(document.getElementsByClassName('ribbon-menu')[0]);
...
window.M = {
    ...
    loadCatTree: function () {
        var mt = J.$('menutree');
        J.load(J.$('menutree'), window._path + '?module=category&action=tree&for=archives&siteid=&ajax=1&rd=' + Math.random() + '#noload', function (result) {
            //var left = mt.getElementsByTagName('B')[0].innerHTML.replace(/<[^>]+>/, '').length * 14 + 30;
            //J.$('sites').style.left = left + 'px';
        });
    },
...
function setSite(id) {
    J.xhr.request({ uri: window._path + '?module=system&action=selectsite&json=1&siteid=' + id, data: 'json' },
        {
...
function newDialog(id, title, url, isAjax, width, height, closeCall) {
    var d = cms.dialog(title, true, true);
    d.onclose = closeCall;
    window['dialog_' + id] = d;
