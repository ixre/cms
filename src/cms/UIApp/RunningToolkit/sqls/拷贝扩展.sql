...
SELECT id as relationId, 1 as relationType,1+4 as fieldId,t2.fieldValue FROM
...
INNER JOIN lms_categories c ON a.cid = c.id 
where relationType=1 and fieldId=1 and lft>=24 and rgt<=39 and c.siteid=1  order by relationId asc) t2
 ON t.title=t2.title
