...
namespace JR.Cms.WebImpl.WebManager.Handle
{
    public class ResourceHandler:BasePage
    {
        ...
        public void Read()
        {
			...
			*/
            ...
            //String resName =Encoding.UTF8.GetString(Convert.FromBase64String(context.Request.Query("res")));
            ...
            Request.TryGetHeader("If-Modified-Since", out var sinceModified);
    ...
    response.Cache.SetSlidingExpiration(true);
                */
                ...
                var filename = Encoding.UTF8.GetString(Convert.FromBase64String(Request.Query("res")));
                ...
                if (Regex.IsMatch(mime, "^image\\/(jpg|png|gif)$", RegexOptions.IgnoreCase))
                {
                    ...
                    else
                    {
                        ...
                        {
                            var file = new FileInfo(string.Format("{0}/{1}local/logo.gif",
                                EnvUtil.GetBaseDirectory(), CmsVariables.FRAMEWORK_PATH));
                            if (file.Exists)
                            {
                                using (var bit = new Bitmap(file.FullName))
                                {
                        ...
                        using (var bit = WebManagerResource.ResourceManager.GetObject(filename) as Bitmap)
                        {
                            var stream = new MemoryStream();
                            bit.Save(stream, ImageFormat.Gif);
                            bit.Dispose();
                ...
                //HttpContextAdapter.Current.Raw.Response.AddHeader("Content-Disposition", "attachment;filename=cms");
            }
        }
        ...
        public void Combine()
        {
            ...
            CacheUtil.Output(Response, int.MaxValue, () =>
            {
                var path = Request.Query("loc").ToString() ?? "";
                var type = Request.Query("res_combine").ToString() ?? "js";
                ...
                foreach (var file in files)
                    if (File.Exists(appDir + file))
                    {
                        ...
                        {
                           Response.WriteAsync(BasePage.CompressHtml(File.ReadAllText(appDir + file)));
                        }
                        else
                        {
                            var bytes = File.ReadAllBytes(appDir + file);
                            Response.Write(bytes, 0, bytes.Length);
                        }
                    }
