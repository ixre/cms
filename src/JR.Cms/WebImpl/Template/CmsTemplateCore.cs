...
namespace JR.Cms.WebImpl.Template
{
    public abstract class CmsTemplateCore : IDisposable
    {
            ...
            if (binds.Length == 2 && binds[1] != String.Empty)
            {
                if (binds[0] == "category")
                {
                    CategoryDto category =
                        ServiceCall.Instance.SiteService.GetCategory(this._site.SiteId, int.Parse(binds[1]));
                    if (category.ID > 0)
                    {
        ...
        protected string Bind(string field)
        {
            ...
            PropertyInfo p = Array.Find(archivePros,
            ...
            string content = TplEngine.FieldTemplate(cmEditorTpl, a =>
            {
                switch (a)
                {
                    ...
                    case "html":
                        return html;
                    case "img":
                        return this.FormatPageUrl(UrlRulePageKeys.Common,
        ...
        protected string Comment(string format, string asc, string usePager)
        {
            ...
            ArchiveDto archive = Cms.Context.Items["archive"] == null
                ? default(ArchiveDto)
        ...
        protected string Form(string formId)
        {
            ...
            //存入SESSION
            HttpContext.Current.Session[String.Format("cms_form_{0}_token", tb.Id.ToString())] = token;
            ...
            sb.Append("<div class=\"fbtn\"><input type=\"button\" id=\"cms_form_")
                ...
                .Append(this.FormatPageUrl(UrlRulePageKeys.Common,
        ...
        {
            ArchiveDto archiveDto =
                ServiceCall.Instance.ArchiveService.GetSameCategoryNextArchive(this.SiteId, int.Parse(id));
            ...
            sb.Append(TplEngine.FieldTemplate(format,
                field =>
                {
                    switch (field)
                    {
                        ...
                        case "title":
                            return !this.FlagAnd(archiveDto.Flag, BuiltInArchiveFlags.IsSpecial)
                                ? archiveDto.Title
            ...
            foreach (DataRow dr in drs)
            {
                ...
                sb.Append(TplEngine.FieldTemplate(format,
                    field =>
                    {
                        ...
                        {
                            case "title":
                                return !this.FlagAnd(Convert.ToInt32(dr["flag"]), BuiltInArchiveFlags.IsSpecial)
                                    ? dr["title"].ToString()
                            ...
                            case "outline":
                                return ArchiveUtility.GetOutline(
                                    String.IsNullOrEmpty((dr["outline"] ?? "").ToString())
                                        ? dr["content"].ToString()
                                        : dr["outline"].ToString(), this.GetSetting().CfgOutlineLength);
            ...
            string format)
        {
            ...
            ArchiveDto[] dt =
                ServiceCall.Instance.ArchiveService.GetSpecialArchives(this.SiteId, category.Path, container, intNum,
                    skipSize);
            ...
            out int recordCount)
        {
            ...
            else
            {
                ...
                this.SetPager(
                    ...
                    pages,
                    total,
                    this.FormatPageUrl(UrlRulePageKeys.Search, HttpUtility.UrlEncode(keyword),
                        categoryTagOrModuleId ?? ""),
                    this.FormatPageUrl(UrlRulePageKeys.SearchPager, HttpUtility.UrlEncode(keyword),
                    ...
                    switch (field)
                    {
                        ...
                        case "class":
                            if (navIndex == i)
                            {
                                return i == 0
                                    ? " class=\"current first\""
                                    : (!isLast ? " class=\"current\"" : " class=\"current last\"");
                            }
                            else
                            {
            ...
            foreach (string tag in tagArr)
            {
                sb.Append(TplEngine.FieldTemplate(format, a =>
                {
                    switch (a)
                    {
                        ...
                        //搜索页URL
                        case "searchurl":
                            return this.FormatPageUrl(UrlRulePageKeys.Search, HttpUtility.UrlEncode(tag), String.Empty);
        ...
        public string Category_Tree(string categoryTag)
        {
            ...
            //读取缓存
            string cacheKey = String.Format("{0}_site{1}_tree_{2}", CacheSign.Category.ToString(), SiteId.ToString(),
                categoryTag);
