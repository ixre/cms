...
function JR() {
    ...
    // 功能
    this.fn = {
        _eventArray: ["abort", "blur", "change", "click", "dblclick",
...
JR.prototype = {
    ...
    param: function (key, url) {
        var match = new RegExp('(\\?|&)' + key + '=([^&]+)&*').exec(url || location.href);
...
JR.prototype.xhr = {
    ...
    get: function (url, success, err) {
        this._doRequest(url instanceof Object ? this.getUrl(url) : { url: url }, this._callback(success, err));
    },
