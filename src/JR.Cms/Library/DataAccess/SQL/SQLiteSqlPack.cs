namespace JR.Cms.Library.DataAccess.SQL
{
    public class SQLiteSqlPack : SqlPack
    {
        ...
        public override string Archive_GetAllArchive =>
            @"SELECT $PREFIX_archive.id,str_id,[alias],[cat_id],title,$PREFIX_archive.location,thumbnail,author_id,
...
$PREFIX_archive.flag,outline,
        ...
        public override string Archive_GetArchivesByModuleId =>
            @"SELECT $PREFIX_archive.id,str_id,[alias],[cat_id],$PREFIX_archive.[flag],
                    ...
                    WHERE " + SqlConst.Archive_NotSystemAndHidden +
            @" AND site_id=@siteId AND ModuleID=@ModuleID ORDER BY $PREFIX_archive.sort_number DESC LIMIT 0,{0}";
        ...
        public override string Archive_GetArchivesByModuleIDAndViewCountDesc =>
            @"SELECT $PREFIX_archive.id,[cat_id],$PREFIX_archive.flag,str_id,[alias],title,
        ...
        public override string Archive_GetSpecialArchivesByModuleID =>
            @"SELECT $PREFIX_archive.id,str_id,[alias],[cat_id],$PREFIX_archive.flag,title,$PREFIX_archive.location,
        ...
        public override string Archive_GetPreviousArchive =>
            @"SELECT [ID],str_id,[alias],a.[cat_id],title,a.[flag],a.location,thumbnail,a.[create_time],a.sort_number FROM $PREFIX_archive a,
        ...
        public override string ArchiveGetPagedArchivesByCategoryIdPagerquery =>
            ...
            @" ORDER BY $PREFIX_archive.sort_number DESC LIMIT $[skipsize],$[pagesize])
        ...
        public override string Archive_GetPagedOperations =>
            "SELECT * FROM $PREFIX_operation LIMIT $[skipsize],$[pagesize]";

        public override string Message_GetPagedMessages =>
            "SELECT * FROM $PREFIX_Message WHERE Recycle=0 AND $[condition] ORDER BY [SendDate] DESC LIMIT $[skipsize],$[pagesize]";

        public override string Member_GetPagedMembers =>
            @"SELECT $PREFIX_member.[id],[username],[avatar],[nickname],[RegIp],[RegTime],[LastLoginTime] FROM $PREFIX_member INNER JOIN $PREFIX_memberdetails
        ...
        public override string ArchiveGetPagedSearchArchivesByCategoryId =>
            @"SELECT $PREFIX_archive.id AS id,* FROM  $PREFIX_archive INNER JOIN 
        ...
        public override string Archive_GetPagedOperationsByAvialble =>
            "SELECT * FROM $PREFIX_operation WHERE $[condition] LIMIT  $[skipsize],$[pagesize]";

        public override string Archive_GetArchivesByCondition =>
            @"SELECT $PREFIX_archive.id,str_id,[alias],[cat_id],title,$PREFIX_archive.location,[Tags],
