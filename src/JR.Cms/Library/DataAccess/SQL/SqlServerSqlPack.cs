    ...
    public class SqlServerSqlPack : SqlPack
    {
        ...
        public override string Archive_GetSelfAndChildArchiveExtendValues =>
            @"
                        ...
                        FROM $PREFIX_archive INNER JOIN $PREFIX_category ON $PREFIX_category.id=$PREFIX_archive.cat_id
                        WHERE $PREFIX_archive.cat_id IN($[catIdArray]) AND $PREFIX_archive.site_id=@siteId  AND " + SqlConst
                                                                                                                      .Archive_NotSystemAndHidden
        ...
        public override string Archive_GetArchivesByModuleId =>
            @"SELECT TOP {0} $PREFIX_archive.id,str_id,[alias],$PREFIX_archive.flag,title,$PREFIX_archive.location,[Outline],
                        ...
                        WHERE " + SqlConst.Archive_NotSystemAndHidden +
            @" AND site_id=@siteId AND ModuleID=@ModuleID ORDER BY $PREFIX_archive.sort_number DESC";

        public override string Archive_GetArchivesByViewCountDesc =>
            @"SELECT TOP {0} $PREFIX_archive.id,$PREFIX_category.id as cat_id,
        ...
        public override string Archive_GetArchivesByModuleIDAndViewCountDesc =>
            @"SELECT TOP {0} $PREFIX_archive.id,[cat_id],$PREFIX_archive.flag,str_id,[alias],title,$PREFIX_archive.location,[Outline],thumbnail,[Content],
                    ...
                    WHERE " + SqlConst.Archive_NotSystemAndHidden +
            @" AND site_id=@siteId AND ModuleID=@ModuleID ORDER BY view_count DESC";
        ...
        public override string Archive_GetSpecialArchivesByModuleID =>
            @"SELECT TOP {0} $PREFIX_archive.id,str_id,[alias],[cat_id],$PREFIX_archive.flag,
        ...
        public override string Archive_GetFirstSpecialArchiveByCategoryID =>
            "SELECT TOP 1 * FROM $PREFIX_archive WHERE [cat_id]=@CategoryId AND site_id=@siteId AND "
            + SqlConst.Archive_Special + @" ORDER BY $PREFIX_archive.sort_number DESC";

        public override string Archive_GetPreviousArchive =>
            @"SELECT TOP 1 [ID],str_id,[alias],a.[cat_id],title,a.flag,a.location,thumbnail,a.[create_time],a.sort_number FROM $PREFIX_archive a,
        ...
        public override string ArchiveGetPagedArchivesByCategoryIdPagerquery =>
            /*
                ...
                */
            @"SELECT * FROM (SELECT $PREFIX_archive.*,
        ...
        public override string Member_GetPagedMembers =>
            ...
            @"SELECT * FROM (SELECT $PREFIX_member.[id],[username],[avatar],[nickname],[RegIp],[RegTime],[LastLoginTime],
        ...
        public override string Archive_GetPagedOperationsByAvialble =>
            "SELECT * FROM (SELECT *,ROW_NUMBER()OVER(ORDER BY id) as rowNum FROM $PREFIX_operation WHERE $[condition]) _t  WHERE rowNum BETWEEN $skipsize AND ($skipsize+$pagesize)";

        public override string Archive_GetArchivesByCondition =>
            @"SELECT $PREFIX_archive.id,str_id,[alias],[cat_id],title,$PREFIX_archive.location,[Tags],[Outline],thumbnail,[Content],[IsSystem],[IsSpecial],[Visible],create_time FROM $PREFIX_archive INNER JOIN $PREFIX_category ON
        ...
        public override string Comment_GetCommentsForArchive =>
            "SELECT * FROM $PREFIX_comment LEFT JOIN $PREFIX_member ON [MemberID]=$PREFIX_member.[ID]" +
        ...
        public override string TableGetLastedRowId => "SELECT TOP 1 id FROM $PREFIX_table_row ORDER BY id DESC";
