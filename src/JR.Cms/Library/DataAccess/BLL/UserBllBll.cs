...
namespace JR.Cms.Library.DataAccess.BLL
{
    public class UserBllBll : IUserBll
    {
        ...
        {
            var result = _dal.CreateOperation(name, path, true);
        ...
        {
            foreach (var o in GetOperationList())
                if (func(o))
                    return o;
            return null;
        }
        ...
        public IList<Operation> GetOperationList()
        {
            if (_operations == null)
                ...
                {
                    if (reader.HasRows) _operations = reader.ToEntityList<Operation>();
                });
            return _operations;
        }
        ...
        {
            return _dal.GetPagedAvailableOperationList(available, pageSize, currentPageIndex, out recordCount,
                out pageCount);
        }
        ...
        public UserGroup[] GetUserGroups()
        {
            if (_groups == null)
            {
                ...
                //todo:
                var tb = new DataTable(); //= _dal.GetUserRoles();
        ...
        {
            var users = _dal.GetAllUser().ToEntityList<User>();
        ...
        public User GetUser(string username)
        {
            ...
            {
                if (rd.HasRows) user = rd.ToEntity<User>();
            });
