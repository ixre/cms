...
namespace JR.Cms.Library.DataAccess.BLL
{
    public class ModuleBLL : Imodule
    {
        ...
        {
            if (GetModule(siteID, name) == null)
            {
                //
                //todo:check siteid
                //

                var result = dal.AddModule(new Module
                {
                    SiteId = siteID,
                    Name = name,
                    IsSystem = false,
        ...
        {
            var result = dal.UpdateModule(module);
            if (result) WeakRefCache.RebuiltModule();
            return result;
        }
        ...
        {
            foreach (var m in WeakRefCache.Modules)
        ...
        public string GetModuleJson()
        {
            ...
            foreach (var m in GetModules())
                if (!m.IsDelete)
                    sb.Append("{\"id\":\"").Append(m.ID.ToString()).Append("\",\"name\":\"").Append(m.Name)
                        .Append("\"},");
