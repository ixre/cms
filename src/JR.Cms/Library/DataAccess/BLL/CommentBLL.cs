...
namespace JR.Cms.Library.DataAccess.BLL
{
    ...
    public sealed class CommentBll : IComment
    {
        ...
        private static ICommentDAL dal => _dal ?? (_dal = new CommentDal());

        private static IRenewsDAL renewDal => _renewDal ?? (_renewDal = new ReviewsDAL());
        ...
        {
            return (archiveId + (agree ? "agree" : "disagree")).ShortMd5();
        }
        ...
        public void InsertComment(string archiveId, int memberId, string ip, string content)
        {
            ...
            if (!isExists) dal.Add(archiveId, memberId, ip, content, false);
        }
        ...
        {
            var id = CreateReviewsID(archiveId, agree);
            var members = renewDal.GetReviewsMembers(id);

            //如果没有数据的话，则创建
            if (members == null) renewDal.CreateReviews(id);
            //如果已经评论
            ...
            //记录点评过的用户
            renewDal.UpdateReviews(id,
                string.IsNullOrEmpty(members) ? memberId.ToString() : members + "," + memberId.ToString());
