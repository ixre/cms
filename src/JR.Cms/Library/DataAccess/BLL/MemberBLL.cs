...
namespace JR.Cms.Library.DataAccess.BLL
{
    ...
    public sealed class MemberBll : Imember
    {
        ...
        {
            var bytes = Encoding.UTF8.GetBytes(username + "," + DateTime.Now.ToString("{yyyy/MM/dd HH:mm:ss}"));
        ...
        public int Regsiter(string username, string password, string nickname, string email, string ip, bool needActive)
        {
            ...
            var uid = dal.GetMemberUid(username);
        ...
        public Member VerifyMember(string username, string password)
        {
            ...
            {
                if (reader.HasRows) m = reader.ToEntity<Member>();
            });
            return m;
        }
        ...
        {
            dal.Update(member.ID, member.Password, member.Nickname, member.Avatar, member.Sex, member.Email,
                member.TelePhone, member.Note);
        }
