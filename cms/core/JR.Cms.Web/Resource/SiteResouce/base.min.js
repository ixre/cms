...
jr.event.add(window, 'load', function () {
    ...
    jr.each(document.getElementsByClassName('ui-exchange'), function (i, e) {
        ...
        var g = 'exchange'; switch (e.nodeName) { case 'IMG': f = 'src'; break; default: f = 'innerHTML'; break } if (f == null) return; v = e[f]; d = e.getAttribute(g); if (d) {
...
function tabCard(e, opt) {
    ...
    var tabItems = jr.$fn(e).find("." + opt.tabClass);
    var tabFrames = jr.$fn(opt.frames).find("." + opt.frameClass);
    var len = tabItems.len();
    tabItems.each(function (i, e) {
        var func = function () {
            var t = this.elem();
            tabItems.each(function (i, e) {
                var fr = tabFrames.get(i);
                if (e.elem() == t) {
                    e.addClass("current");
                    fr.removeClass("hidden");
                    if (opt.call != null) opt.call(e.elem(), fr.elem());
                } else {
