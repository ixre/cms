...
namespace Ops.Cms.DAL
{
    ...
    public sealed class MessageDAL:DALBase,ImessageDAL
    {
        ...
        {
            base.ExecuteReader(
                new SqlQuery(base.OptimizeSQL(SP.Message_GetMessage),
                    new object[,]{{"@id", id}
                    }),
                func
                );
        ...
        public void WriteMessage(int sendUID, int receiveUID, string subject, string content)
        {
            base.ExecuteNonQuery(
               new SqlQuery(base.OptimizeSQL(SP.Mesage_InsertMessage),
                   new object[,]{
               {"@SendUId", sendUID},
        ...
        public int SetRead(int receiveUid, int id)
        {
            return base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Message_SetRead),
                    new object[,]{
                {"@ReceiveUId", receiveUid},
                {"@id", id}
                    }));
        ...
        {
            return base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Message_SetRecycle),new object[,]{
                {"@ReceiveUId",receiveUid},
                {"@id", id}
                }));
        }
        ...
        {
            return base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Message_Delete),
                    new object[,]{
                 {"@ReceiveUId", receiveUid},
                 {"@id", id}
                    }));
        ...
        public DataTable GetPagedMessage(int uid, int typeID, int pageSize, ref int currentPageIndex, out int recordCount, out int pageCount)
        {
            ...
            recordCount = int.Parse(base.ExecuteScalar(new SqlQuery(
                String.Format(base.OptimizeSQL(SP.Message_GetPagedMessagesCount), condition),null)
                ).ToString());
            ...
            string sql = skipCount == 0 && base.DbType == DataBaseType.OLEDB ?
                        ...
                        base.OptimizeSQL(SP.Message_GetPagedMessages);
