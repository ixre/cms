...
namespace AtNet.Cms.DAL
{
    ...
    public sealed class CommentDAL:DALBase,ICommentDAL
    {
        ...
        {
            base.ExecuteNonQuery(new SqlQuery(base.OptimizeSQL(SP.Comment_AddComment),
                new object[,]{
                {"@ArchiveId", archiveID},
                {"@MemberId",memberID},
                {"@IP",ip},
        ...
        public int GetArchiveCommentsCount(string archiveID)
        {
            return int.Parse(base.ExecuteScalar(
                new SqlQuery(base.OptimizeSQL(SP.Comment_QueryCommentsCountForArchive),
                    new object[,]{
                 {"@ArchiveId", archiveID}
                    })
                 ).ToString());
        ...
        public void Delete(int commentID)
        {
            base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Comment_Delete),
                    new object[,]{
                {"@id", commentID}
                    })
                );
        ...
        {
            return base.GetDataSet(new SqlQuery(base.OptimizeSQL(SP.Comment_GetCommentsForArchive) + (desc ? " ORDER BY createdate DESC" : ""),
                new object[,]{
                 {"@ArchiveId", archiveID}
                })).Tables[0];
        }
        ...
        public DataTable GetCommentsDetailsTable(string archiveID)
        {
            return base.GetDataSet(
                new SqlQuery(base.OptimizeSQL(SP.Comment_GetCommentDetailsListForArchive),
                    new object[,]{
        ...
        {
            base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Comment_DeleteMemberComments),
                    new object[,]{
                {"@id", id}
                    })
            );
        ...
        {
            base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Comment_DeleteArchiveComments), 
                    new object[,]{
                {"@ArchiveId", archiveID}
                    }));
        }
