    ...
    internal class CmsInstallWiz
    {
        ...
        public InstallCode Process(HttpContext context)
        {
            ...
            if (db_type == "sqlite")
            {
                if (db_file == "")
                {
                    db_file ="rd_"+StringExtensions.RandomLetters(string.Empty,5)+".db";
                }
            ...
            else if (db_type == "oledb")
            {
                if (db_file == "")
                {
                    db_file = "rd_" + StringExtensions.RandomLetters(string.Empty,5) + ".mdb";
                }
                else if (db_file.IndexOf(".") == -1)
                {
                    db_file += ".mdb";
            ...
            this.db.ExecuteNonQuery(new SqlQuery(
                String.Format(@"
                    ...
                    ", db_prefix),
                     new object[,]{
                         {"@username",user_name},
                         {"@password",  StringExtensions.EncodeMD5(StringExtensions.Encode16MD5(user_pwd))},
                         {"@name","管理员"},
                         {"@dt",DateTime.Now}
                     }));
