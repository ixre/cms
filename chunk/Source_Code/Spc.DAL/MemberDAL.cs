    ...
    public sealed class MemberDAL:DALBase,ImemberDAL
    {
        public bool DetectUserName(string username)
        {
            object obj = base.ExecuteScalar(
                new SqlQuery(base.OptimizeSQL(SP.Member_DetectUsername),
                    new object[,]{
                {"@Username", username}
                    }));
            return obj == null;
        ...
        {
            object obj = base.ExecuteScalar(
                new SqlQuery(base.OptimizeSQL(SP.Member_DetectNickname),
                    new object[,]{
                {"@Nickname", nickname}
                    }));
            return obj == null;
        ...
        {
            object obj = base.ExecuteScalar(
                new SqlQuery(base.OptimizeSQL(SP.Member_DetectUsernameAndNickNameHasExits),
                    new object[,]{
                {"@Username", username},
                {"@Nickname", nickname}
                    }));
        ...
        public void RegisterMember(string username,string password,string avatar,string sex,string nickname,string note,string email,string telephone)
        {
            base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Member_RegisterMember),
                    new object[,]{
                {"@Username",username},
                {"@Password",password},
                {"@Avatar",""},
        ...
        {
            base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Member_InsertMemberDetails),
                    new object[,]{
                {"@UId",memberID},
                {"@Status",status},
                {"@RegIP",regIP},
        ...
        {
            object obj= base.ExecuteScalar(
                new SqlQuery(base.OptimizeSQL(SP.Member_GetMemberUID),
                    new object[,]{
                {"@Username", username}
                    })
                );
        ...
        {
            base.ExecuteReader(
                new SqlQuery(base.OptimizeSQL(SP.Member_VerifyMember),
                    new object[,]{
{"@Username", username},
                {"@Password", password}
                    }),
        ...
        {
            base.ExecuteReader(
                new SqlQuery(base.OptimizeSQL(SP.Member_GetMemberByUsername),
                    new object[,]{
                       {"@Username", username}
                    }),
                func
        ...
        {
            base.ExecuteReader(
                  new SqlQuery(base.OptimizeSQL(SP.Member_GetMemberInfoAndDetails),
                      new object[,]{
                        {"@id",-1},
                        {"@Username", username}
                      }),
        ...
        {
            base.ExecuteNonQuery(
                new SqlQuery(base.OptimizeSQL(SP.Member_Update),
                    new object[,]{
                {"@Password",password},
                {"@Avatar", avatar},
                 {"@Sex",sex},
        ...
        public void Delete(int memberID)
        {
            ...
            base.ExecuteNonQuery(new SqlQuery(base.OptimizeSQL(SP.Member_DeleteMember),parameters),
                        new SqlQuery(base.OptimizeSQL(SP.Member_DeleteMemberDetails), parameters)); ;
        }
        ...
        {
            //ACCESS时候第一页执行
            const string sql1 = @"SELECT TOP $[pagesize] [id],[username],[avatar],[nickname],[RegIp],[RegTime],[LastLoginTime] FROM $PREFIX_members INNER JOIN $PREFIX_MemberDetails ON $PREFIX_members.[ID]=$PREFIX_MemberDetails.[UID]";

            recordCount = int.Parse(
                   base.ExecuteScalar(new SqlQuery(base.OptimizeSQL(SP.Member_GetMemberCount))).ToString()
                );
            ...
            string sql = skipCount == 0 && base.DbType== DataBaseType.OLEDB ?
                        ...
                        base.OptimizeSQL(SP.Member_GetPagedMembers);
