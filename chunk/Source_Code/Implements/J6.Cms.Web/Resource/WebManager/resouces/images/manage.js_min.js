...
function autoCompletion(v, w, x, y, z, A) { var B; var C; this.charMinLen = A || 1; this.lastChar = ''; this.isOnFocus = false; this.timer = null; this.url = w; if (!v.nodeName) { v = j6.$(v) } var D = function () { B = v.previousNode; if (!B || B.nodeName != 'DIV' || B.className != 'ui-autocompletion-panel') { if (v.parentNode.offsetLeft > v.offsetLeft) { v.parentNode.style.cssText += 'position:relative' } B = document.createElement('DIV'); B.className = 'ui-autocompletion-panel'; B.style.cssText = 'curcor:default;z-index:102;position:absolute;left:' + v.offsetLeft + 'px;top:' + (v.offsetTop + v.offsetHeight) + 'px;width:' + v.offsetWidth + 'px;overflow:hidden;display:none'; v.parentNode.insertBefore(B, v); C = document.createElement('DIV'); C.className = 'inner'; C.style.cssText = 'background-color:#fff;'; B.appendChild(C) } else { C = B.getElementsByTagName('DIV')[0] } }; D(); var E = (function (e, p, q, r, s, u, t) { return function (l, m) { if (m) t.isOnFocus = true; var n = window.event || l; if (n.altKey || n.keyCode == 17) return; var o = e.value; if (/^\s*$/.test(o) && t.charMinLen != 0) return; o = o.replace(/^(\s*)(.+?)(\s*)$/, '$2'); if (o.length < t.charMinLen) return; if (t.lastChar != '' && t.lastChar == o) { return } else { t.lastChar = o } j6.xhr.request({ uri: t.url + (t.url.indexOf('?') == -1 ? "?" : '&') + 'key=' + encodeURIComponent(o), params: {}, method: 'GET', data: 'json' }, { success: function (d) { if (r) r(d); if (d.length != 0) { p.style.display = ''; var f = new RegExp(o, 'i'); var g = '<b>' + o + '</b>'; var h = '<ul style="margin:0;padding:0;">'; for (var i = 0; i < d.length; i++) { h += '<li' + (i == 0 ? ' class="first"' : '') + (d[i].title ? ' title="' + d[i].title + '"' : '') + '>' + d[i].text.replace(f, g) + '</li>'; if (d[i].text == o && s) { if (e.onblur) e.onblur(); s(d[i]) } } h += '</ul>'; q.innerHTML = h; var k = q.getElementsByTagName('LI'); j6.each(k, function (i, c) { c.onmouseover = (function (a, b) { return function () { for (var j = 0; j < b.length; j++) { b[j].className = j == 0 ? 'first' : '' } this.className = this == k[0] ? 'first selected' : 'selected' } })(p, k); c.onclick = (function (j, a) { return function () { e.value = j.text; a.style.display = 'none'; if (e.onblur) e.onblur(); if (s) s(j) } })(d[i], p) }) } else { p.style.display = 'none' } setTimeout(function () { t.isOnFocus = false }, 500) }, error: function () { if (u && u instanceof Function) u() } }) } })(v, B, C, x, y, z, this); var F = (function (p, t) { return function (a) { if (!t.isOnFocus) { p.style.display = 'none' } } })(B, this); j6.event.add(v, 'focus', (function (t) { return function (a) { E(a, true) } })(this)); j6.event.add(v, 'keyup', E); j6.event.add(document, 'click', F); return this } j6.extend({ autoCompletion: function (a, b, c, d, e, f) { return new autoCompletion(a, b, c, d, e, f) } });
...
j6.event.add(window, 'load', function () { j6.each(document.getElementsByClassName('ui-exchange'), function (i, e) { var v = null; var d = null; var f = null; var g = 'exchange'; switch (e.nodeName) { case 'IMG': f = 'src'; break; default: f = 'innerHTML'; break } if (f == null) return; v = e[f]; d = e.getAttribute(g); if (d) { j6.event.add(e, 'mouseover', (function (a, b, c) { return function () { a[b] = c } })(e, f, d)); j6.event.add(e, 'mouseout', (function (a, b, c) { return function () { a[b] = c } })(e, f, v)) } }) });
...
function showMsg(msg, callback, second, notMask) {
    ...
    if (!win.xhrCt) {
        ...
        win.xhrCt.innerHTML = j6.template('<div class="msg" id="xhr_msg_layout"></div>', {
            h: j6.screen.height()
        });
    ...
    var mw = msg.replace(/<[^>]+>/ig, '').length * 12 + 20;
    win.xhrMsg.innerHTML = msg;
    win.xhrCt.style.width = mw + 'px';
...
j6.dialog.ALERT_ALL_TITLE = "J6-CMS";
...
//加载地址到div中
j6.loadurl = function (id, url, func) {
    var e = null;
    if (id.nodeName) {
        e = id;
    } else {
...
//按模块加载页面
j6.load2 = function (id, module, action, query, func) {
    ...
    j6.lazyRun(function () {
        j6.loadurl(id, url, func);
    });
};

//加载分页数据
j6.pagerLoad = function (id, pid, page, module, action, query, func) {
    var e = j6.$(id), pe = j6.$(pid), eDis = e.style.display;
    ...
    j6.loadurl(id, url, function (result) {
...
//=======================================================================================//
j6.propertyUpload = function (id, id2) {
    window['pro_upload_' + id] = j6.upload({
        id: id,
        debug: !true,
        ...
        document.onkeydown = function (event) {
            ...
            if (e.ctrlKey && e.keyCode == 83) {
                if (window.saveData && window.saveData instanceof Function) {
                    window.saveData();
                    return j6.event.preventDefault(e);
                }
                return true;
            }
                //按键ALT+F11,启用全屏
...
//初始化
j6.event.add(window, 'load', window.bindInit);
