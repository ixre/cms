...
function autoCompletion(u, v, w, x, y) { var z; var A; this.charMinLen = (y == 0 ? 0 : 1); this.lastChar = ''; this.isOnFocus = false; this.timer = null; if (!u.nodeName) { u = j6.$(u) } var B = function () { z = u.previousNode; if (!z || z.nodeName != 'DIV' || z.className != 'ui-autocompletion-panel') { if (u.parentNode.offsetLeft > u.offsetLeft) { u.parentNode.style.cssText += 'position:relative' } z = document.createElement('DIV'); z.className = 'ui-autocompletion-panel'; z.style.cssText = 'curcor:default;z-index:102;position:absolute;left:' + u.offsetLeft + 'px;top:' + (u.offsetTop + u.offsetHeight) + 'px;width:' + u.offsetWidth + 'px;overflow:hidden;display:none'; u.parentNode.insertBefore(z, u); A = document.createElement('DIV'); A.className = 'inner'; A.style.cssText = 'background-color:#fff;'; z.appendChild(A) } else { A = z.getElementsByTagName('DIV')[0] } }; B(); var C = (function (e, p, q, r, s, t) { return function (l, m) { if (m) t.isOnFocus = true; var n = window.event || l; if (n.altKey || n.keyCode == 17) return; var o = e.value; if (/^\s*$/.test(o) && t.charMinLen != 0) return; o = o.replace(/^(\s*)(.+?)(\s*)$/, '$2'); if (o.length < t.charMinLen) return; if (t.lastChar != '' && t.lastChar == o) { return } else { t.lastChar = o } j6.xhr.request({ uri: v + (v.indexOf('?') == -1 ? "?" : '&') + 'key=' + encodeURIComponent(o), params: {}, method: 'GET', data: 'json' }, { success: function (d) { if (r) r(d); if (d.length != 0) { p.style.display = ''; var f = new RegExp(o, 'i'); var g = '<b>' + o + '</b>'; var h = '<ul style="margin:0;padding:0;">'; for (var i = 0; i < d.length; i++) { h += '<li' + (i == 0 ? ' class="first"' : '') + (d[i].title ? ' title="' + d[i].title + '"' : '') + '>' + d[i].text.replace(f, g) + '</li>'; if (d[i].text == o && s) { if (e.onblur) e.onblur(); s(d[i]) } } h += '</ul>'; q.innerHTML = h; var k = q.getElementsByTagName('LI'); j6.each(k, function (i, c) { c.onmouseover = (function (a, b) { return function () { for (var j = 0; j < b.length; j++) { b[j].className = j == 0 ? 'first' : '' } this.className = this == k[0] ? 'first selected' : 'selected' } })(p, k); c.onclick = (function (j, a) { return function () { e.value = j.text; a.style.display = 'none'; if (e.onblur) e.onblur(); if (s) s(j) } })(d[i], p) }) } else { p.style.display = 'none' } setTimeout(function () { t.isOnFocus = false }, 500) } }) } })(u, z, A, w, x, this); var D = (function (p, t) { return function (a) { if (!t.isOnFocus) { p.style.display = 'none' } } })(z, this); j6.event.add(u, 'focus', (function (t) { return function (a) { C(a, true) } })(this)); j6.event.add(u, 'keyup', C); j6.event.add(document, 'click', D); return this } j6.extend({ autoCompletion: function (a, b, c, d, e) { return new autoCompletion(a, b, c, d, e) } });
...
function showMsg(msg, callback, second, notMask) {
    ...
    if (!win.xhrCt) {
        ...
        win.xhrCt.innerHTML = j6.template('<div class="gate" style="width:{w}px;height:{h}px" id="xhr_gate_layout"></div><div class="msg" id="xhr_msg_layout"></div>', {
            w: j6.screen.width(),
            h: j6.screen.height()
        });
    ...
    win.xhrGate.className = notMask ? "hidden" : '';
...
j6.xhr.filter = function (s, method, c) {
    if (s === 0) {
        showMsg('<span class="load"></span>请求中', method === "GET");
    } else if (s === 2) {
