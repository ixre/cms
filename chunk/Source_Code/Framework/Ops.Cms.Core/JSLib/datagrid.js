...
function datagrid(ele, config) {
    this.panel = ele.nodeName ? ele : $JS.$(ele);
    ...
    this._initLayout = function () {
        ...
        this.gridView = (this.panel.getElementsByClassName
            ...
            : $JS.getElementsByClassName(this.panel, 'ui-datagrid-view'))[0];

        this.loadbox = this.gridView.getElementsByTagName('DIV')[0];
    };

    this._resize = function () {
        $JS.dom.fitHeight(this.panel);
    };
    ...
    //    };
    //})(this);
    //$JS.event.add(window, 'load', resizeFunc);
    //window.attachEvent('resize', resizeFunc);
    //$JS.event.add(window, 'resize', this.resize.apply(this));
