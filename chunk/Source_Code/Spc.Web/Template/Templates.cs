...
namespace Ops.Cms.Template
{
    ...
    public partial class CmsTemplates : CmsTemplateDataMethod
    {
        ...
        {
            return base.Archive(this.site.SiteId, idOrAlias, format);
        }
        ...
        {
            string id = HttpContext.Current.Items["archive.id"] as string;
            if (String.IsNullOrEmpty(id))
            {
                return this.TplMessage("Error: 此标签只能在文档页面中调用!");
            }
            return Archive(id, format);
        }
        ...
        {
            string cacheKey = String.Format("{0}_site{1}_navigator", CacheSign.Link.ToString(), this.siteId.ToString());

            return Cms.Cache.GetCachedResult(cacheKey, () =>
            {
                return base.Navigator(base.TplSetting.CFG_NavigatorLinkFormat, base.TplSetting.CFG_NavigatorChildFormat, "-1");
            });
        }
        ...
        {
            string cacheKey = String.Format("{0}_site{1}_friendlink_{2}", CacheSign.Link.ToString(), this.siteId.ToString(), num);
            return Cms.Cache.GetCachedResult(cacheKey, () =>
            {
                return this.Link("2", format, int.Parse(num), "-1");
            });
        }
