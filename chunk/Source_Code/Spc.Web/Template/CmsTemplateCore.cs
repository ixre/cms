...
namespace Ops.Cms.Template
{
    ...
    public abstract class CmsTemplateCore : IDisposable
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public virtual string GetArchiveUrl(string flags, CategoryDto category, string id)
        {
            //if (category == null)
            //{
            //    Archive archive = CmsLogic.Archive.Get(id);
            //   category = CmsLogic.Category.Get(a => a.ID == archive.Cid);
            //}
        ...
        protected string GetBingLinkUrl(string bindStr)
        {
            ...
            else
            {
                ...
                else if (binds[0] == "archive")
                {
                    ...
                    if (archive.Id > 0)
                    {
                        return this.GetArchiveUrl(archive.Flags,
                            archive.Category,
                            String.IsNullOrEmpty(archive.Alias)
                            ? archive.StrId : archive.Alias);
                    }
                }
            }
        ...
        {
            return str == "1" || String.Compare(str, "true", true) == 0;
        }
        ...
        protected string GetThumbnailUrl(string thumbnail)
        {
            ...
            if (!Settings.TPL_UseFullPath           //不使用完整路径
                || thumbnail.IndexOf("://") != -1)    //如果是包含协议的地址
            {
                return thumbnail;
            }
            else
            {
                ...
                {
                    string resourceDomain = Cms.Context.ResourceDomain;
                    if (resourceDomain != String.Empty)
                    {
                        resourceDomain = resourceDomain.Substring(0, resourceDomain.Length - 1);
                    }
                    this.resourceUri = resourceDomain;
                }
        ...
        protected string Navigator(string format, string childFormat, string index)
        {
            ...
            LinkBehavior bh = (link) =>
            {
                ...
                //解析格式
                tempLinkStr = tplengine.FieldTemplate(isChild ? childFormat : format, a =>
                {
                    switch (a)
                    {
                ...
                if (links[i].Pid == 0)
                {
                    ...
                    if (childs.Count != 0)
                    {
                        phtml = phtml.Replace("</li>",
                            String.Format("<div class=\"child\" id=\"link_{0}_{1}\"><ul class=\"menu\">",
                            links[i].Type.ToString(), links[i].ID.ToString())
                            );
        ...
        /// <returns></returns>
        protected string Archive(int siteId, string idOrAlias, string format)
        {
            ArchiveDto archive = ServiceCall.Instance.ArchiveService.GetArchiveByIdOrAlias(siteId, idOrAlias);

            if (archive.Id <= 0) return TplMessage(String.Format("不存在编号（或别名）为:{0}的文档!", idOrAlias));


            StringBuilder sb = new StringBuilder(500);
            this.FormatArchive(sb, archive, format, null, -1);
            return sb.ToString();
        }
        ...
        {
            ArchiveDto archive = ServiceCall.Instance.ArchiveService.GetSameCategoryNextArchive(this.siteId, int.Parse(id));

            if (!(archive.Id > 0)) return Cms.Language.Get(LanguagePackageKey.ARCHIVE_NoNext);
        ...
        protected virtual string ArchiveList(IEnumerable<ArchiveDto> archives, string format)
        {
            ...
            {
                this.FormatArchive(sb, archive, format, archives, tmpInt++);
            }
            return sb.ToString();
        }
        ...
        protected string PagerArchives(string categoryTag, string pageIndex, string pageSize, string format)
        {
            ...
            int i = 0;
            DataRowCollection drs = ServiceCall.Instance.ArchiveService.GetPagedArchives(
                this.siteId,
                category.Lft,
                category.Rgt,
                _pageSize,
                ...
                sb.Append(tplengine.FieldTemplate(format,
                    field =>
                    {
                        switch (field)
                        {
                            ...
                            //链接
                            case "url":
                                return GetArchiveUrl(dr["flags"].ToString(), archiveCategory, id);
                            ...
                            // 项目顺序类
                            case "class":
                                if (i == drs.Count - 1) return " class=\"last\"";
                                else if (i == 0) return " class=\"first\"";
                                return string.Empty;
        ...
        protected string SearchArchives(int siteId,string categoryTagOrModuleId, string keyword, string pageIndex, string pageSize, string format, out int pageCount, out int recordCount)
        {
                ...
                sb.Append(tplengine.FieldTemplate(format,
                    field =>
                    {
                        switch (field)
                        {
                            ...
                            //链接
                            case "url":
                                return GetArchiveUrl(archive.Flags, category, alias);
            ...
            if (sb.Length < 30)
            {
                sb.Append(String.Format("<div class=\"noresult\">没有相关记录！</div>", keyword));
            }
            else
            {
                //设置分页
        ...
        {
            string cacheKey = String.Concat(CacheSign.Link.ToString(),
                "_s", this.siteId.ToString(),
                "_",
                type.ToString(),
                number.ToString()
                );

            return CacheFactory.Sington.GetCachedResult<String>(cacheKey, () =>
            {
                int _type = 2;
        ...
        protected string CategorySelectTree(string categoryTag, string split)
        {
            ...
            //缓存
            Cms.Cache.Insert(cacheKey, result);
