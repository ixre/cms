...
namespace Ops.Cms.WebManager
{
    ...
    public class C_Ajax : BasePage
    {
        ...
        internal string GetAppInit()
        {
            ...
            model.User usr = UserState.Administrator.Current;
            ...
            if (notice == null)
            {
                try
                {
                    WebRequest request = WebRequest.Create("http://ct.ops.cc/view/opsite_notice/");
                    ...
                    foreach (XmlNode tn in tl)
                    {
                        if (tn.Name == "link")
                        {
                            ...
                            sb.Append("{text:'").Append(tn.Attributes["text"].Value)
                                .Append("',uri:'").Append(tn.Attributes["href"].Value.Replace("$prefix",Ops.Cms.Conf.Settings.SYS_ADMIN_TAG))
                                .Append("'}");
                        }
                    }
        ...
        {
            string title = base.Request["word"];
